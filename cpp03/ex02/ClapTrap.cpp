...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/24 16:38:01 by nmeintje          #+#    #+#             */
/*   Updated: 2025/03/27 11:06:29 by nmeintje         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(std::string name, int HP, int EP, int AD)
{
	...
	std::cout << YELLOW << "Test constructor called! " << DEFAULT << std::endl;
}
...
void ClapTrap::takeDamage(unsigned int amount) 
{
	...
	{
		_hitPoints -= amount;
		std::cout << GREEN << "ClapTrap " << _name << " has lost " << amount << " Hit Points!"
			  << DEFAULT << std::endl;
	}
	else if ((int)amount > _hitPoints)
	{
		_hitPoints = 0;
		std::cout << GREEN << "ClapTrap " << _name << " has no more Hit Points! :-["
			  << DEFAULT << std::endl;
	}
}
...
void ClapTrap::beRepaired(unsigned int amount) 
{
	if (_energyPoints >= 1 && _hitPoints >= 1)
	{
		...
		std::cout << GREEN << "ClapTrap " << _name << " has recovered " << amount
